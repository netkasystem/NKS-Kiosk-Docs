...
window.Step17 = {
    ...
    init: async () => {
        ...
        if (!getIntegrateNdppData?.length) {
            next_page();
            return;
        }
        ...
        if (form && !form.dataset.bound) {
            form.addEventListener("submit", async (e) => {
                e.preventDefault();
                await Step17.submitConsent(cardData);
            });
        ...
        if (!container.dataset.bound) {
            container.addEventListener("click", async (e) => {
                ...
                Step17._ndppId = btn.getAttribute("data-id");
                Step17._url = btn.getAttribute("data-url");
    ...
    showNdppForm: (data) => {
        ...
        if (purposeOptions) {
            ...
            (data.purpose_option || []).forEach(purpose => {
                ...
                purposeOptions.appendChild(div);
            });
        }
    ...
    submitConsent: async (cardData) => {
        const firstname = cardData?.thaiPersonalInfo?.firstName || "";
        const lastname = cardData?.thaiPersonalInfo?.lastName || "";
        const email = Step17._previousConsent?.email || "";

        const selectedPurposes = Array.from(
            document.querySelectorAll("#ndpp-form-new input[name='purpose']:checked")
        ).map(input => parseInt(input.value));

        const purposeOptionDetail = Array.from(
            document.querySelectorAll("#ndpp-form-new input[name='purpose']")
        ).map(input => ({
